...
then
    ...
    echo "Failover is complete. You successfully migrated from $primaryInstace in $primaryZone to $drInstance in $drZone, and recreated replicas in the following zones:"
    for zone in "${replicaZones[@]}"
    do
        echo $zone
    done
